...
веб-панели Timeweb** при создании сервера и **вход в браузере** (открыть ссылку с
VPN и прислать код обратно в чат). Всё остальное делает Claude — ты только пишешь.
...
Честно про предел этой изоляции: она держится **на уровне сайта**, а не на уровне
операционной системы. Сама программа Claude и терминал работают от главного
пользователя сервера (root). Технически подкованный человек через терминал сможет
дотянуться до чужой папки. Для **своей доверенной команды** — нормально. Для
чужих людей с улицы — нет. Подробнее про это — в
...
| Папки людей | `/srv/cloudcli-users/<логин>` | личный загон каждого: его проекты и чаты |
...
- **Изоляция — на уровне сайта, не железа.** Каждый заперт в своей папке в панели,
  но через терминал подкованный человек дотянется до чужого. Для доверенных
  коллег — ок.
