...
Твои руки нужны только в двух местах на всём сервере: клики в панели [Timeweb](https://timeweb.cloud/?i=144973) при создании сервера и открытие ссылки для входа в браузере с VPN (скопировать код и вставить его обратно в чат). Всё остальное делает Claude.
...
> Важно: изоляция стоит на панели плюс на правах доступа к папкам (`0700` у каждого). Посторонняя программа на сервере чужое не прочитает, но панель и Claude работают от root, а root видит всё — изнутри панели подкованный человек до чужой папки дотянуться может. Для доверенной команды это нормально, для чужих людей — нет. Подробности честных ограничений — в [01-how-it-works.md](./01-how-it-works.md).

---

## Блок 4а. Надо убедиться, что чужие файлы никому не видны

Повод бывает такой: на сервер поставили ещё одну программу (например, бота), и надо понять, дотянется ли она до рабочих папок команды.

**Что написать Claude:** «Проверь, что папки людей на сервере закрыты». Claude прогонит проверку и ответит одним из двух: все папки закрыты (права `0700`, у каждой свой владелец), либо список открытых папок — тогда он тут же их закроет и покажет проверку заново.

> Под капотом Claude выполнит: `cd /root/claudecode-kit-src && bash secure-workspaces.sh --check`, а если проверка нашла открытые — `bash secure-workspaces.sh`.

**Claude сообщит тебе:** сколько папок проверено и что каждая закрыта. Файлы внутри папок он не трогает — закрывается сама папка, и этого хватает: внутрь чужой закрытой папки посторонний не войдёт.
